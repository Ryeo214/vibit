...
The Vibit Core daemon can be configured to act as a trusted "border
router", implementing the vibit wire protocol and relay, making
...
The ZeroMQ feature in Vibit Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building vibitd:
...
For instance:
    ...
    $ vibitd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/vibitd.tx.raw \
...
These options can also be provided in vibit.conf.
...
From the perspective of vibitd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into vibitd directly. Furthermore, no information is
...
using. Vibitd appends an up-counting sequence number to each
